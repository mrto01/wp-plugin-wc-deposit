...
class Email_Partial_paid extends \WC_Email {
    
    use Email_Trait;
    
    public function __construct() {
        $this->id             = 'depart_partial_paid';
        $this->title          = __( 'DEPART - Partial payment received', 'depart-deposit-and-part-payment-for-woocommerce' );
        $this->description    = __( 'Partial paid emails are sent to customer when an Partial order is paid', 'depart-deposit-and-part-payment-for-woocommerce' );
        $this->customer_email = true;
        $this->template_html  = 'emails/partial-paid.php';
        $this->template_plain = 'emails/plain/partial-paid.php';
...
//        add_action( 'woocommerce_order_status_on-hold_to_completed_notification', array( $this, 'trigger' ), 10, 2 );
    ...
    public function trigger( $order_id, $order = false ) {
        ...
        if ( is_a( $order, 'WC_Order' ) ) {
            ...
            $real_order_remaining = $this->remaining_amount;
            if ( $order->is_paid() ) {
                $real_order_remaining = (int) ( $real_order_remaining - $order->get_total() );
            }
    ...
    public function get_default_subject() {
        return esc_html__( '[{site_title}]: Partial payment #{wdp_suborder_number} has been received!', 'depart-deposit-and-part-payment-for-woocommerce' );
    }
    
    public function get_default_heading() {
        return esc_html__( 'Thank you for your partial payment', 'depart-deposit-and-part-payment-for-woocommerce' );
    }
    
    public function get_default_email_text() {
        return esc_html__( 'Your partial payment [ #{wdp_suborder_number} ] for order #{wdp_parent_order_number} has been received.', 'depart-deposit-and-part-payment-for-woocommerce' );
    }
    
    public function get_default_payment_text() {
        return esc_html__( 'To pay the remaining amount, please visit following link {wdp_order_detail_link}', 'depart-deposit-and-part-payment-for-woocommerce' );
    }
